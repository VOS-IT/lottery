...
public partial class Signup : System.Web.UI.Page
{
    ...
    protected void BtnSignup_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            wsr1 = DBService.IsExistingUser(Email.Value.Trim(),PhoneNumber.Value.Trim());
            if (wsr1.Status == "1")
            {
                ...
                if (wsr.Status == "1")
                {                   
                    wsr2 = DBService.InsertUserInfo(FirstName.Value.Trim(), LastName.Value.Trim(), PhoneNumber.Value, Email.Value.Trim(), Password.Value.Trim(), ReferralBy.Value.Trim());
                    if (wsr2.Status == "1")
                    {
