...
public partial class AddTicket : System.Web.UI.Page
{
    ...
    protected void BtnAddTicket_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            wsr = DBService.InsertTicketInfo(TicketNo.Value.Trim(), int.Parse(TicketPrice.Value.Trim()), int.Parse(PriceAmount.Value.Trim()), DateTime.Parse(DateTime.Now.ToString("yyy-MM-dd")), DateTime.Parse(CloseDate.Value), DateTime.Parse(DrawDate.Value), Status.SelectedItem.Text,Type.SelectedItem.Text);
            if (wsr.Status == "1")
            {
